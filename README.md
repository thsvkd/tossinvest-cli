...
> 커뮤니티 랭킹·뉴스 브리핑·실시간 푸시·소수점 주문·dry-run preview 등)을 이미 제공하고,
...
- **분류** (전체 카탈로그: [`docs/reverse-engineering/wts-endpoints.json`](docs/reverse-engineering/wts-endpoints.json)):
  ...
  - `candidate` / `priority: next` — 아직 미구현, 그중 **다음에 추가하면 좋을 고가치 후보**를 별도 표기 (예: 가상자산·지수 시세, 실시간 차트, 업종(TICS) 랭킹, AI 시그널 상세)
