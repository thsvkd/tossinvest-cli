...
```
config.json 허용 (per-action 토글 + allow_live_order_actions)
  → preview → --execute → --dangerously-skip-permissions → --confirm <token>
```

다단계 게이트. 거래 기능은 기본 전부 꺼져 있고, config 에서 하나씩 열어야 실행 가능.
(v0.4.21: 중복이던 TTL grant 레이어 제거 — `allow_live_order_actions` 마스터
스위치가 같은 보호를 하므로 간소화.)
